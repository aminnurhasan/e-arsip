    ...
    <div class="container-fluid">
        <div class="row">
            <section class="col-lg-12">
                <a href="{{route('createDokumentasiAdminAnggaran')}}" class="btn btn-md btn-info mb-2">Tambah Dokumentasi</a>
                            ...
                            <tbody>
                                @foreach ($dokumentasi as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href='{{ route('showDokumentasiAdminAnggaran', $item->id) }}' class="btn btn-warning btn-sm fas fa-eye"></a>
                                            <a href="{{url('/admin_anggaran/dokumentasi/' . $item->id . '/edit')}}" class="btn btn-warning btn-sm fas fa-pen-to-square"></a>
                                            <form onsubmit="return confirm('Apakah Anda Ingin Menghapus Data ?')" data-confirm-delete="true" class="d-inline" 
                                            action="{{ url('admin_anggaran/dokumentasi/' . $item->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
