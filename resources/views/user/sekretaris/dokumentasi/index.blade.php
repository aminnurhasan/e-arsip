                                ...
                                @foreach ($dokumentasi as $item)
                                    <tr>
                                        ...
                                        <td style="text-align: center">
                                            <a href='{{ route('showDokumentasiSekretaris', $item->id) }}' class="btn btn-warning btn-sm fas fa-eye m-1"></a>
                                            <a href="{{url('/sekretaris/dokumentasi/' . $item->id . '/edit')}}" class="btn btn-warning btn-sm fas fa-pen-to-square m-1"></a>
                                            <form onsubmit="return confirm('Apakah Anda Ingin Menghapus Data ?')" data-confirm-delete="true" class="d-inline m-1" 
                                            action="{{ url('sekretaris/dokumentasi/' . $item->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
